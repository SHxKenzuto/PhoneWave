...
async def player(ctx, voice_client, id):
    if ctx.guild.id in phonewaves:
        ...
        if (
            ...
            and ctx.guild.id in phonewaves
            and phonewaves[ctx.guild.id].loop == False
            and phonewaves[ctx.guild.id].q.empty()
            and phonewaves[ctx.guild.id].latest_player_id == id
        ):
...
async def play(ctx, *, msg):
    ...
    if ctx.guild.id not in phonewaves:
        ...
        if voice_client.is_playing():
            await ctx.send(
                "Song queued in position #{n}.".format(
                    n=phonewaves[ctx.guild.id].q.qsize() + 1
                )
            )
        phonewaves[ctx.guild.id].q.put(url_video)
        phonewaves[ctx.guild.id].latest_player_id = random.random()
...
async def skip(ctx):
    ...
    if (
        ...
        and ctx.message.author.voice.channel == voice_client.channel
    ):
        voice_client.stop()
...
async def loop(ctx):
    ...
    if (
        ...
        await ctx.send(
            "Loop {val}".format(val="ON" if phonewaves[ctx.guild.id].loop else "OFF")
        )
